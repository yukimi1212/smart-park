                ...
                <ul class="nav">
					...
					<li class="nav-item nav-dropdown ">
                        ...
                        <ul class="nav-dropdown-items">       
                            ...
                            <li class="nav-item">
                                <a href="javascript:void(0)" onclick="getType()"  class="nav-link active" id="recordType">
...
<script type="text/javascript">  
	...
	function doRecordTypeSearch(sWord,source){
	...
	function checkSearchWord() {
		...
		if(sWord == "")
			...
			$.ajax({
	     		...
	     		success:function(result){
	     			if(result != "null")
	     				doRecordTypeSearch(sWord,result);
	     		},
	     		error:function(error){
	     			var jsonData = JSON.stringify(error);
	     	    	alert(jsonData)
	...
	function doSearch(){
		...
		if (source == "parkingtype"){
			...
			if(searchWord == "")
				...
				var user_id = $("#user_id").html();
				var param = encode64(user_id);
				var url = "http://localhost:8080/user/" + param + "/" + searchWord + "&recordtype";
		        window.location.href=url;
			}	
		}
	}
	...
	function showParkingTypeData(data) {
		$("#tab").html("");
	...
	function showRecordTypeData(data) {
		$("#tab").html("");
		var str = "<thead><tr><th>停车场类型编号</th><th>类型名</th><th>停车记录数量</th></tr></thead><tbody>";
		for (var i = 1; i < data.length; i++) {
			str = str + "<tr><td>" + data[i].typecode + "</td><td>" + data[i].typename + "</td><td>" + data[i].amount + "</td></tr>"; 
		}
		str = str + "</tbody>";
